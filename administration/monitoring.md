...
| `fluentbit_input_bytes_total` | name: the name or alias for the input instance | The number of bytes of log records that this input instance has ingested successfully. | counter | bytes |
| `fluentbit_input_files_abandoned_bytes_total` | name: the name or alias for the input instance | The cumulative unread raw source-file bytes discarded when monitored files are terminally removed (`rotate_wait` expiration, file deletion, or truncation). Only available for the [Tail](../pipeline/inputs/tail.md) input plugin. | counter | bytes |
...
| `fluentbit_input_files_processed_bytes_total` | name: the name or alias for the input instance | The cumulative raw source-file bytes past which the resumable offset has advanced. Only available for the [Tail](../pipeline/inputs/tail.md) input plugin. | counter | bytes |
