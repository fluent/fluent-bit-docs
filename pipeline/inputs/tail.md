...
The database file essentially stores `inode=offset` so it is recommended to be unique per instance of the plugin, e.g. if you have two tail inputs then use two separate `db` files for each. That way each tail input can independently track its own state.
