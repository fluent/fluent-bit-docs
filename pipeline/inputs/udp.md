...
| `parser`             | Optional [parser](../parsers.md) for line-delimited records. Requires `format` to be `none`; if `format` is set to anything else, it's automatically switched to `none` and a warning is logged.     | _none_                      |
