...
| `mem_buf_limit`                     | Set a memory buffer limit for the input plugin instance in bytes. If the limit is reached, the plugin will pause until the buffer is drained. If set to 0, the buffer limit is disabled. Note that if the plugin has enabled filesystem buffering, this limit won't apply. | `0`      |
