...
Using the [CPU](cpu-metrics.md) input plugin as an example we will flush CPU metrics to [Fluentd](http://fluentd.org):
...
So we gathered [CPU](cpu-metrics.md) metrics and flushed them out to [Fluentd](http://fluentd.org) properly.
...
Secure Forward aims to provide a secure channel of communication with the remote Fluentd service using [TLS](../../administration/security.md). Above there is a minimalist configuration for testing purposes.
