...
In the following example, the [content modifier](../data-pipeline/processors/content-modifier.md) processor inserts or updates (upserts) the key `my_new_key` with the value `123` for all log records generated by the tail plugin. This processor is only applied to logs.
