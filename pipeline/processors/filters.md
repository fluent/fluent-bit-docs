...
In this example configuration, an input plugin uses the [Lua](../data-pipeline/filters/lua.md) filter as a processor to add a new key `hostname` with the value `monox`. Then, an output plugin adds a new key named `output` with the value `new data`.
