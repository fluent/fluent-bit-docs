...
| static\_file\_path | Disables behavior where UUID string is automatically appended to end of S3 key name when `$UUID` is not provided in `s3_key_format`. If this is enabled without a dynamic key format, the previous file will be overwritten. | false |
