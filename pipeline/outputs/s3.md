...
See [AWS Credentials](../../administration/aws-credentials.md)
...
| `endpoint` | Custom endpoint for the S3 API. Endpoints can contain scheme and port. Set this parameter to send data to an S3-compatible object store other than Amazon S3, such as MinIO or Google Cloud Storage. Requests use path-style addressing (`endpoint/bucket/key`) and are signed with the configured `region`. | _none_ |
...
| `quarantine_dir_limit_size` | Maximum disk space for quarantined chunks. Only applies when `retry_exhausted_action` is `quarantine`. When the limit is reached, new retry-exhausted chunks are deleted instead of quarantined. `0` means unlimited. | `0` |
...
| `retry_exhausted_action` | Action for chunks that have exceeded the `retry_limit`. Accepted values: `delete` (permanently remove the chunk) or `quarantine` (move to a quarantine area and exclude from active buffer accounting). | `quarantine` |
...
The S3 output must sometimes fall back to the [`PutObject` API](https://docs.aws.amazon.com/AmazonS3/latest/API/API_PutObject.html).
...
[MinIO](https://www.min.io/) is a high-performance, S3 compatible object storage, and you can build your app with S3 capability without S3.
