...
Fluent Bit 1.7 adds a new feature called `workers` which enables outputs to have dedicated threads. This `s3` plugin has partial support for workers. **The plugin can only support a single worker in multipart upload mode (the default)**; enabling multiple workers will lead to errors/indeterminate behavior. If you set `use_put_object On` then the plugin can support any number of workers.
