...
| time\_key\_format | strftime compliant format string for the timestamp; for example, the default is '%Y-%m-%dT%H:%M:%S'. Support millisecond and nanosecond with '%3N' and '%L'; for example, adding '%3N' to support millisecond '%Y-%m-%dT%H:%M:%S:%3N'. This option is used with time\_key. |
