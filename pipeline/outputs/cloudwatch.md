...
| `extra_user_agent`                 | This option appends a string to the default user agent. AWS asks that you not manually set this field yourself, it's reserved for use in vended configurations, for example, EKS Container Insights.| _none_     |
...
| `log_group_template`               | Template for CW Log Group name using record accessor syntax. Plugin falls back to the `log_group_name` configured if needed.                                                                              | _none_     |
...
| `region`                           | The AWS region to send logs to. China regions (`cn-north-1`, `cn-northwest-1`) and the AWS European Sovereign Cloud region (`eusc-de-east-1`) are supported; Fluent Bit automatically uses the correct endpoint suffix (`.amazonaws.com.cn` or `.amazonaws.eu`) and no custom `endpoint` is required. | _none_     |
