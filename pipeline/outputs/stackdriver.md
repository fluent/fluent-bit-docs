...
| location | The GCP or AWS region in which to store data about the resource. Used for _generic\_node_ and _generic\_task_.|  |
| namespace | A namespace identifier, such as a cluster name or environment. Used for _generic\_node_ and _generic\_task_.|  |
| node_id | A unique identifier for the node within the namespace, such as hostname or IP address. Used for _generic\_node_.|  |
| job | An identifier for a grouping of related task, such as the name of a  microservice or distributed batch. Used for _generic\_task_.|  |
| task_id | A unique identifier for the task within the namespace and job, such as a replica index identifying the task within the job. Used for _generic\_task_.|  |
